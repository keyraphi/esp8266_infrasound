...
function updateCharts(dataJson) {
  ...
  for (let i = 0; i < timeArrayMS.length; i++) {
    ...
    if(chartTimeSeries.series[0].data.length > 30 * 60) {
  ...
  movingSequence = movingSequence.slice(-fft_window);
  var fft_time_sequence = Array.from(movingSequence)
  if (fft_time_sequence.length < fft_window) {
    var padding = new Array(fft_window - fft_time_sequence.length).fill(0)
